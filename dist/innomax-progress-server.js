...
exports.default = innomaxProgressRouter;
const express_1 = __importDefault(require("express"));
function innomaxProgressRouter(pool) {
    const router = express_1.default.Router();
    // ==============================
    // 유틸: JSON 값 안전하게 읽기
    // ==============================
    function safeText(obj, key, def = "") {
        if (!obj || typeof obj[key] === "undefined" || obj[key] === null)
            return def;
        return String(obj[key]);
    }
    function safeNumber(obj, key, def = 0) {
        if (!obj || typeof obj[key] === "undefined" || obj[key] === null)
            return def;
    ...
    router.get("/my-works", async (req, res) => {
        ...
        try {
            ...
            rows.forEach((row, index) => {
                ...
                // employees 배열 확인
                const employees = (_a = row.detail_json) === null || _a === void 0 ? void 0 : _a.employees;
            ...
            const result = rows.map(row => {
                ...
                const myLogs = logs.filter((x) => x.user_id === userId);
    ...
    router.post("/work/progress-update/:currentWorkId", async (req, res) => {
        ...
        try {
            ...
            const { work_id, user_id, date, progress_status, progress_percent, report_text, attached_file_url, } = req.body;
            console.log("📡 [progress/save] params =", currentWorkId);
            console.log("📡 [progress/save] body =", req.body);
            // ⚠️ 파라미터 ID와 body 의 work_id 불일치 대비
            const finalWorkId = work_id !== null && work_id !== void 0 ? work_id : currentWorkId;
            // 1) 기존 detail_json 조회
            const q1 = `
            SELECT detail_json
            FROM innomax_works
        ...
        `;
            ...
            // 진행상황 버퍼 생성
            if (!detail.progress_buffer)
                detail.progress_buffer = {};
            // 날짜 버퍼 생성
